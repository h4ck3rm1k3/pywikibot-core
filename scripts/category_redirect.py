...
class CategoryRedirectBot(object):
    ...
    def change_category(self, article, oldCat, newCat, comment=None,
        ...
        try:
            ...
            if newtext == oldtext:
                pywikibot.output('No changes made in page %s.'
            ...
            pywikibot.output('Skipping locked page %s'
                             % article.title(asLink=True))
            ...
            pywikibot.output(
                'Changing page %s blocked by spam filter (URL=%s)'
    ...
    def get_log_text(self):
        ...
        keep = [text for (key, text) in
                sorted(iter(log_items.items()), reverse=True)[:LOG_SIZE - 1]]
    ...
    def run(self):
        ...
        try:
            ...
            pywikibot.output("No redirect templates defined for %s"
                             % self.site.sitename())
            return
        ...
        for cat in pagegenerators.PreloadingGenerator(nonemptypages):
            ...
            if not self.readyToEdit(cat):
                ...
                self.log_text.append("* Skipping %s; in cooldown period."
                                     % cat.title(asLink=True, textlink=True))
                continue
