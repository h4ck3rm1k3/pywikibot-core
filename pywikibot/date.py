...
_knDigits = '೦೧೨೩೪೫೬೭೮೯'
_knDigitsToLocal = dict([(ord(str(i)), _knDigits[i]) for i in range(10)])
_knLocalToDigits = dict([(ord(_knDigits[i]), str(i)) for i in range(10)])

# Helper for Urdu/Persian languages
_faDigits = '۰۱۲۳۴۵۶۷۸۹'
_faDigitsToLocal = dict([(ord(str(i)), _faDigits[i]) for i in range(10)])
_faLocalToDigits = dict([(ord(_faDigits[i]), str(i)) for i in range(10)])

# Helper for HI:, MR:
_hiDigits = '०१२३४५६७८९'
_hiDigitsToLocal = dict([(ord(str(i)), _hiDigits[i]) for i in range(10)])
_hiLocalToDigits = dict([(ord(_hiDigits[i]), str(i)) for i in range(10)])

# Helper for BN:
_bnDigits = '০১২৩৪৫৬৭৮৯'
_bnDigitsToLocal = dict([(ord(str(i)), _bnDigits[i]) for i in range(10)])
_bnLocalToDigits = dict([(ord(_bnDigits[i]), str(i)) for i in range(10)])

# Helper for GU:
_guDigits = '૦૧૨૩૪૫૬૭૮૯'
_guDigitsToLocal = dict([(ord(str(i)), _guDigits[i]) for i in range(10)])
_guLocalToDigits = dict([(ord(_guDigits[i]), str(i)) for i in range(10)])
...
_digitDecoders = {
    ...
    'T': (_decimalDigits, lambda v: str(v+543), lambda v: int(v)-543),
}
...
formats = {
    'MonthName': {
        'af': lambda v: slh(v, ["Januarie", "Februarie", "Maart", "April",
                                ...
                                "Desember"]),
        'als': lambda v: slh(v, ["Januar", "Februar", "März", "April",
        ...
        'bn': lambda v: slh(v, ["জানুয়ারি", "ফেব্রুয়ারি", "মার্চ", "এপ্রিল", "মে",
        ...
        'eo': lambda v: slh(v, ["Januaro", "Februaro", "Marto", "Aprilo",
                                ...
                                "Decembro"]),
        'es': lambda v: slh(v, ["enero", "febrero", "marzo", "abril",
        ...
        'et': lambda v: slh(v, ["jaanuar", "veebruar", "märts", "aprill",
                                ...
                                "detsember"]),
        'eu': lambda v: slh(v, ["Urtarril", "Otsail", "Martxo", "Apiril",
                                "Maiatz", "Ekain", "Uztail", "Abuztu",
                                "Irail", "Urri", "Azaro", "Abendu"]),
        ...
        'fi': lambda v: slh(v, ["tammikuu", "helmikuu", "maaliskuu",
                                ...
                                "marraskuu", "joulukuu"]),
        'fo': lambda v: slh(v, ["januar", "februar", "mars", "apríl",
                                "mai", "juni", "juli", "august",
                                "september", "oktober", "november",
                                "desember"]),
        ...
        'fur': lambda v: slh(v, ["Zenâr", "Fevrâr", "Març", "Avrîl", "Mai",
                                 "Jugn", "Lui", "Avost", "Setembar",
                                 "Otubar", "Novembar", "Dicembar"]),
        'fy': lambda v: slh(v, ["jannewaris", "febrewaris", "maart",
                                "april", "maaie", "juny", "july",
        ...
        'hi': lambda v: slh(v, ["जनवरी", "फ़रवरी", "मार्च", "अप्रैल", "मई", "जून",
        ...
        'jv': lambda v: slh(v, ["Januari", "Februari", "Maret", "April",
                                "Mei", "Juni", "Juli", "Agustus",
                                "September", "Oktober", "November",
                                "Desember"]),
        'ka': lambda v: slh(v, ["იანვარი", "თებერვალი", "მარტი", "აპრილი",
                                ...
                                "დეკემბერი"]),
        'kn': lambda v: slh(v, ["ಜನವರಿ", "ಫೆಬ್ರವರಿ", "ಮಾರ್ಚಿ", "ಎಪ್ರಿಲ್", "ಮೇ",
                                "ಜೂನ", "ಜುಲೈ", "ಆಗಸ್ಟ್", "ಸೆಪ್ಟೆಂಬರ್", "ಅಕ್ಟೋಬರ್",
                                "ನವೆಂಬರ್", "ಡಿಸೆಂಬರ್"]),
        ...
        'ksh': lambda v: slh(v, ['Jannowaa', 'Febrowaa', 'Mä', 'Apprill',
                                 ...
                                 'Dezemmber']),
        'ku': lambda v: slh(v, ["rêbendan", "reşemî", "adar", "avrêl",
                                "gulan", "pûşper", "tîrmeh",
                                "gelawêj (meh)", "rezber", "kewçêr",
                                "sermawez", "berfanbar"]),
        ...
        'la': lambda v: slh(v, ["Ianuarius", "Februarius", "Martius",
                                ...
                                "November", "December"]),
        'lb': lambda v: slh(v, ["Januar", "Februar", "Mäerz", "Abrëll",
                                "Mee", "Juni", "Juli", "August",
                                "September", "Oktober", "November",
                                "Dezember"]),
        'li': lambda v: slh(v, ["jannewarie", "fibberwarie", "miert",
                                "april", "mei", "juni", "juli",
                                "augustus (maond)", "september", "oktober",
                                "november", "december"]),
        ...
        'mhr': lambda v: slh(v, ["шорыкйол", "пургыж", "ӱярня", "вӱдшор",
                                 "ага", "пеледыш", "сӱрем", "сорла",
                                 "идым", "шыжа", "кылме", "декабрь"]),
        'mi': lambda v: slh(v, ["Kohi-tātea", "Hui-tanguru",
                                "Poutū-te-rangi", "Paenga-whāwhā", "Haratua",
                                "Pipiri", "Hōngongoi", "Here-turi-kōkā",
                                "Mahuru", "Whiringa-ā-nuku",
                                "Whiringa-ā-rangi", "Hakihea"]),
        'ml': lambda v: slh(v, ["ജനുവരി", "ഫെബ്രുവരി", "മാര്ച്",
                                "ഏപ്രില്", "മേയ്", "ജൂണ്‍", "ജൂലൈ",
                                "ആഗസ്റ്റ്‌", "സപ്തന്പര്", "ഒക്ടോബര്",
                                "നവന്പര്", "ഡിസന്പര്"]),
        'mr': lambda v: slh(v, ["जानेवारी", "फेब्रुवारी", "मार्च", "एप्रिल", "मे",
                                "जून", "जुलै", "ऑगस्ट", "सप्टेंबर", "ऑक्टोबर",
                                "नोव्हेंबर", "डिसेंबर"]),
        'ms': lambda v: slh(v, ["Januari", "Februari", "Mac", "April",
        ...
        'nap': lambda v: slh(v, ["Jennaro", "Frevaro", "Màrzo", "Abbrile",
                                 ...
                                 "Dicembre"]),
        'nds': lambda v: slh(v, ["Januar", "Februar", "März", "April",
                                 "Mai", "Juni", "Juli", "August",
                                 "September", "Oktober", "November",
                                 "Dezember"]),
        'nl': lambda v: slh(v, ["januari", "februari", "maart", "april",
                                "mei", "juni", "juli", "augustus (maand)",
                                "september", "oktober", "november",
                                "december"]),
        'nn': lambda v: slh(v, ["januar", "februar", "månaden mars",
                                "april", "mai", "juni", "juli", "august",
                                "september", "oktober", "november",
                                "desember"]),
        'no': lambda v: slh(v, ["januar", "februar", "mars", "april",
                                "mai", "juni", "juli", "august",
                                "september", "oktober", "november",
                                "desember"]),
        'oc': lambda v: slh(v, ["genièr", "febrièr", "març", "abril",
                                "mai", "junh", "julhet", "agost",
                                "setembre", "octobre", "novembre",
                                "decembre"]),
        'os': lambda v: slh(v, ["январь", "февраль", "мартъи", "апрель",
                                "май", "июнь", "июль", "август",
                                "сентябрь", "октябрь", "ноябрь",
                                "декабрь"]),
        'pdc': lambda v: slh(v, ['Yenner', 'Hanning', 'Matz', 'Abril',
                                 'Moi', 'Yuni', 'Yuli', 'Aagscht',
                                 'September', 'Oktower', 'Nowember',
                                 'Disember']),
        'pl': lambda v: slh(v, ["styczeń", "luty", "marzec", "kwiecień",
        ...
        'pt': lambda v: slh(v, ["Janeiro", "Fevereiro", "Março", "Abril",
                                ...
                                "Setembro", "Outubro", "Novembro",
                                "Dezembro"]),
        'ro': lambda v: slh(v, ["ianuarie", "februarie", "martie",
                                "aprilie", "mai", "iunie", "iulie",
                                "august", "septembrie", "octombrie",
                                "noiembrie", "decembrie"]),
        'ru': lambda v: slh(v, ["январь", "февраль", "март", "апрель",
                                "май", "июнь", "июль", "август",
                                "сентябрь", "октябрь", "ноябрь",
                                "декабрь"]),
        ...
        'sco': lambda v: slh(v, ["Januar", "Februar", "Mairch", "Aprile",
                                 "Mey", "Juin", "Julie", "August",
                                 "September", "October", "November",
                                 "December"]),
        'se': lambda v: slh(v, ["ođđajagimánnu", "guovvamánnu",
                                "njukčamánnu", "cuoŋománnu", "miessemánnu",
                                "geassemánnu", "suoidnemánnu", "borgemánnu",
                                "čakčamánnu", "golggotmánnu", "skábmamánnu",
                                "juovlamánnu"]),
        'simple': lambda v: slh(v, ["January", "February", "March",
                                    "April", "May", "June", "July",
        ...
        'sl': lambda v: slh(v, ["januar", "februar", "marec", "april",
                                "maj", "junij", "julij", "avgust",
                                "september", "oktober", "november",
                                "december"]),
        'sq': lambda v: slh(v, ["Janari", "Shkurti", "Marsi (muaj)",
                                "Prilli", "Maji", "Qershori", "Korriku",
                                "Gushti", "Shtatori", "Tetori", "Nëntori",
                                "Dhjetori"]),
        'sr': lambda v: slh(v, ["јануар", "фебруар", "март", "април",
                                "мај", "јун", "јул", "август", "септембар",
                                "октобар", "новембар", "децембар"]),
        'su': lambda v: slh(v, ["Januari", "Pébruari", "Maret", "April",
                                "Méi", "Juni", "Juli", "Agustus",
                                "Séptémber", "Oktober", "Nopémber",
                                "Désémber"]),
        'sv': lambda v: slh(v, ["januari", "februari", "mars", "april",
                                "maj", "juni", "juli", "augusti",
                                "september", "oktober", "november",
                                "december"]),
        'ta': lambda v: slh(v, ["ஜனவரி", "பிப்ரவரி", "மார்ச்", "ஏப்ரல்",
                                "மே", "ஜூன்", "ஜூலை", "ஆகஸ்டு",
                                "செப்டம்பர்", "அக்டோபர்", "நவம்பர்",
                                "டிசம்பர்"]),
        'te': lambda v: slh(v, ["జనవరి", "ఫిబ్రవరి", "మార్చి", "ఏప్రిల్", "మే",
                                "జూన్", "జూలై", "ఆగష్టు", "సెప్టెంబర్", "అక్టోబర్",
                                "నవంబర్", "డిసెంబర్"]),
        'th': lambda v: slh(v, ["มกราคม", "กุมภาพันธ์", "มีนาคม", "เมษายน",
                                "พฤษภาคม", "มิถุนายน", "กรกฎาคม", "สิงหาคม",
                                "กันยายน", "ตุลาคม", "พฤศจิกายน", "ธันวาคม"]),
        'tl': lambda v: slh(v, ["Enero", "Pebrero", "Marso", "Abril",
                                "Mayo", "Hunyo", "Hulyo", "Agosto",
                                "Setyembre", "Oktubre", "Nobyembre",
                                "Disyembre"]),
        'tpi': lambda v: slh(v, ["Janueri", "Februeri", "Mas", "Epril",
                                 "Me", "Jun", "Julai", "Ogas", "Septemba",
                                 "Oktoba", "Novemba", "Disemba"]),
        'tr': lambda v: slh(v, ["Ocak", "Şubat", "Mart", "Nisan", "Mayıs",
                                "Haziran", "Temmuz", "Ağustos", "Eylül",
                                "Ekim", "Kasım", "Aralık"]),
        'tt': lambda v: slh(v, ["Ğínwar", "Febräl", "Mart", "Äpril", "May",
                                "Yün", "Yül", "August", "Sentäber",
                                "Öktäber", "Nöyäber", "Dekäber"]),
        'uk': lambda v: slh(v, ["січень", "лютий", "березень", "квітень",
                                "травень", "червень", "липень", "серпень",
                                "вересень", "жовтень", "листопад",
                                "грудень"]),
        'ur': lambda v: slh(v, ["جنوری", "فروری", "مارچ", "اپريل", "مئ",
        ...
        'vo': lambda v: slh(v, ["Yanul", "Febul", "Mäzul", "Prilul",
                                "Mayul", "Yunul", "Yulul", "Gustul",
                                "Setul", "Tobul", "Novul", "Dekul"]),
        'wa': lambda v: slh(v, ["djanvî", "fevrî", "Måss (moes)", "avri",
                                "may", "djun", "djulete", "awousse",
                                "setimbe", "octôbe", "nôvimbe", "decimbe"]),
        'zh': lambda v: slh(v, makeMonthList("%d月")),
        'zh-min-nan': lambda v: slh(v, ["It-goe̍h", "Jī-goe̍h", "Saⁿ-goe̍h",
                                        "Sì-goe̍h", "Gō·-goe̍h",
                                        "La̍k-goe̍h", "Chhit-goe̍h",
                                        "Peh-goe̍h", "Káu-goe̍h",
    ...
    'Number': {
        ...
        'no': lambda v: dh_number(v, '%d (tall)'),
    ...
    'YearAD': {
        ...
        'zh-min-nan': lambda v: dh_yearAD(v, '%d nî'),
    },
    ...
    'YearBC': {
        ...
        'no': lambda v: dh_yearBC(v, '%d f.Kr.'),
        ...
        'simple': lambda v: dh_yearBC(v, '%d BC'),
    ...
    'DecadeAD': {
        ...
        'no': lambda v: dh_decAD(v, '%d-årene'),
        ...
        'zh-min-nan': lambda v: dh_decAD(v, '%d nî-tāi'),
    },
    ...
    'DecadeBC': {
        ...
        'ksh': lambda v: dh_decBC(v, '%d-er Joohre füür Krėßtůß'),  # uncertain if that's right. might go to redirect.
    ...
    'CenturyAD': {
        ...
        'als': lambda v: dh_centuryAD(v, '%d. Jahrhundert'),
        ...
        'nn': lambda m: multi(m, [
            ...
            (lambda v: dh(v, '%d00-talet',
                          lambda i: i - 1, lambda ii: ii[0] + 1), alwaysTrue)]),
        'no': lambda v: dh_centuryAD(v, '%d. århundre'),
        ...
        'zh-min-nan': lambda v: dh_centuryAD(v, '%d sè-kí'),
    },
    ...
    'CenturyBC': {
        ...
        'ksh': lambda v: dh_centuryBC(v, '%d. Joohunndot füür Kreůßtůß'),  # uncertain if that's right. might go to redirect.
        ...
        'nn': lambda m: multi(m, [
            (lambda v: dh_constVal(v, 1, '1. århundret fvt.'),
             lambda p: p == 1),
            (lambda v: dh(v, '%d00-talet fvt.',
                          lambda i: i - 1, lambda ii: ii[0] + 1), alwaysTrue)]),
        'no': lambda v: dh_centuryBC(v, '%d. århundre f.Kr.'),
    ...
    'CenturyBC_Cat': {
        ...
        'no': lambda v: dh(v, '%d-tallet f.Kr.',
                           lambda i: (i-1) * 100, lambda ii: ii[0]/100 + 1),
    },
    ...
    'MillenniumAD': {
        ...
        'ksh': lambda m: multi(m, [
            ...
            (lambda v: dh_constVal(v, 5, 'Föfte Johdousend'),
             lambda p: p == 5),
            (lambda v: dh_milleniumAD(v, '%d. Johdousend'), alwaysTrue)]),
    ...
    'MillenniumBC': {
        ...
        'fi': lambda m: multi(m, [
            (lambda v: dh_constVal(v, 1, 'Ensimmäinen vuosituhat eaa.'),
             lambda p: p == 1),
            (lambda v: dh(v, '%d000-vuosituhat eaa.',
                          lambda i: i - 1, lambda ii: ii[0] + 1), alwaysTrue)]),
    ...
    'Cat_Year_MusicAlbums': {
        ...
        'no': lambda v: dh_yearAD(v, 'Musikkalbum fra %d'),
    ...
    'Cat_BirthsAD': {
        ...
        'be-x-old': lambda v: dh_yearAD(v, 'Нарадзіліся ў %d годзе'),
        ...
        'no': lambda v: dh_yearAD(v, 'Fødsler i %d'),
        ...
        'simple': lambda v: dh_yearAD(v, '%d births'),
        ...
        'zh-yue': lambda v: dh_yearAD(v, '%d年出世'),
    },
    ...
    'Cat_DeathsAD': {
        ...
        'be-x-old': lambda v: dh_yearAD(v, 'Памерлі ў %d годзе'),
        ...
        'no': lambda v: dh_yearAD(v, 'Dødsfall i %d'),
        ...
        'simple': lambda v: dh_yearAD(v, '%d deaths'),
        ...
        'zh-yue': lambda v: dh_yearAD(v, '%d年死'),
    },

    'Cat_BirthsBC': {
        'en': lambda v: dh_yearBC(v, '%d BC births'),
        'no': lambda v: dh_yearBC(v, 'Fødsler i %d f.Kr.'),
    },
    'Cat_DeathsBC': {
        'en': lambda v: dh_yearBC(v, '%d BC deaths'),
        'fr': lambda v: dh_yearBC(v, 'Décès en -%d'),
        'no': lambda v: dh_yearBC(v, 'Dødsfall i %d f.Kr.'),
    },
    ...
    'CurrEvents': {
        ...
        'la': lambda v: dh_singVal(v, 'Novissima'),
        ...
        'no': lambda v: dh_singVal(v, 'Aktuelt'),
        ...
        'simple': lambda v: dh_singVal(v, 'World news'),
        ...
        'zh-min-nan': lambda v: dh_singVal(v, 'Sin-bûn sū-kiāⁿ'),
    },
}
...
def addFmt(lang, isMnthOfYear, patterns):
    ...
    """
    if len(patterns) != 12:
        raise AssertionError('pattern %s does not have 12 elements' % lang)

    for i in range(12):
        if patterns[i] is not None:
            if isMnthOfYear:
                formats[yrMnthFmts[i]][lang] = eval(
                    'lambda v: dh_mnthOfYear(v, u"%s")' % patterns[i])
            else:
                formats[dayMnthFmts[i]][lang] = eval(
                    'lambda v: dh_dayOfMnth(v, u"%s")' % patterns[i])
...
addFmt2('als', False, "%%d. %s", True)
addFmt ('an', False, ["%d de chinero", "%d de frebero", "%d de marzo",
...
addFmt ('ar', False, ["%d يناير", "%d فبراير", "%d مارس", "%d أبريل",
                      "%d مايو", "%d يونيو", "%d يوليو", "%d أغسطس",
                      "%d سبتمبر", "%d أكتوبر", "%d نوفمبر", "%d ديسمبر"])
addFmt ('ast', False, ["%d de xineru", "%d de febreru", "%d de marzu",
                       "%d d'abril", "%d de mayu", "%d de xunu",
                       "%d de xunetu", "%d d'agostu", "%d de setiembre",
                       "%d d'ochobre", "%d de payares", "%d d'avientu"])
addFmt ('be', False, ["%d студзеня", "%d лютага", "%d сакавіка",
...
addFmt ('ca', False, ["%d de gener", "%d de febrer", "%d de març",
...
addFmt ('co', False, ["%d di ghjennaghju", "%d di frivaghju", "%d di marzu",
                      "%d d'aprile", "%d di maghju", "%d di ghjugnu",
                      "%d di lugliu", "%d d'aostu", "%d di settembre",
...
addFmt ('el', False, ["%d Ιανουαρίου", "%d Φεβρουαρίου", "%d Μαρτίου",
                      "%d Απριλίου", "%d Μαΐου", "%d Ιουνίου",
                      "%d Ιουλίου", "%d Αυγούστου", "%d Σεπτεμβρίου",
                      "%d Οκτωβρίου", "%d Νοεμβρίου", "%d Δεκεμβρίου"])
...
addFmt ('fa', False, ["%d ژانویه", "%d فوریه", "%d مارس", "%d آوریل",
                      "%d مه", "%d ژوئن", "%d ژوئیه", "%d اوت",
                      "%d سپتامبر", "%d اکتبر", "%d نوامبر", "%d دسامبر"])
addFmt2('fi', False, "%%d. %sta", False)
addFmt2('fo', False, "%%d. %s", False)
addFmt ('fr', False, ["%d janvier", "%d février", "%d mars", "%d avril",
                      "%d mai", "%d juin", "%d juillet", "%d août",
                      "%d septembre", "%d octobre", "%d novembre",
                      "%d décembre"])
addFmt2('fur', False, "%%d di %s", True)
addFmt2('fy', False, "%%d %s", False)
addFmt ('ga', False, ["%d Eanáir", "%d Feabhra", "%d Márta", "%d Aibreán",
                      "%d Bealtaine", "%d Meitheamh", "%d Iúil",
                      "%d Lúnasa", "%d Meán Fómhair", "%d Deireadh Fómhair",
                      "%d Samhain", "%d Mí na Nollag"])
addFmt2('gl', False, "%%d de %s", False)
addFmt2('he', False, "%%d ב%s")  # [u"%d בינואר", u"%d בפברואר", u"%d במרץ", u"%d באפריל", u"%d במאי", u"%d ביוני", u"%d ביולי", u"%d באוגוסט", u"%d בספטמבר", u"%d באוקטובר", u"%d בנובמבר", u"%d בדצמבר"])
addFmt ('hr', False, ["%d. siječnja", "%d. veljače", "%d. ožujka",
                      "%d. travnja", "%d. svibnja", "%d. lipnja",
                      "%d. srpnja", "%d. kolovoza", "%d. rujna",
                      "%d. listopada", "%d. studenog", "%d. prosinca"])
...
addFmt ('is', False, ["%d. janúar", "%d. febrúar", "%d. mars", "%d. apríl",
                      "%d. maí", "%d. júní", "%d. júlí", "%d. ágúst",
                      "%d. september", "%d. október", "%d. nóvember",
                      "%d. desember"])
addFmt2('it', False, "%%d %s", False)
addFmt ('ja', False, makeMonthList("%d月%%d日"))
addFmt2('jv', False, "%%d %s", True)
addFmt2('ka', False, "%%d %s")
addFmt ('ko', False, makeMonthList("%d월 %%d일"))
addFmt ('ku', False, ["%d'ê rêbendanê", "%d'ê reşemiyê", "%d'ê adarê",
                      "%d'ê avrêlê", "%d'ê gulanê", "%d'ê pûşperê",
                      "%d'ê tîrmehê", "%d'ê gelawêjê", "%d'ê rezberê",
                      "%d'ê kewçêrê", "%d'ê sermawezê", "%d'ê berfanbarê"])
addFmt ('la', False, ["%d Ianuarii", "%d Februarii", "%d Martii",
                      "%d Aprilis", "%d Maii", "%d Iunii", "%d Iulii",
                      "%d Augusti", "%d Septembris", "%d Octobris",
                      "%d Novembris", "%d Decembris"])
addFmt2('lb', False, "%%d. %s", True)
addFmt ('li', False, ["%d januari", "%d februari", "%d miert", "%d april",
                      "%d mei", "%d juni", "%d juli", "%d augustus",
                      "%d september", "%d oktober", "%d november",
                      "%d december"])
addFmt ('lt', False, ["Sausio %d", "Vasario %d", "Kovo %d", "Balandžio %d",
                      "Gegužės %d", "Birželio %d", "Liepos %d",
                      "Rugpjūčio %d", "Rugsėjo %d", "Spalio %d",
                      "Lapkričio %d", "Gruodžio %d"])
addFmt2('lv', False, "%%d. %s", False)
addFmt2('mhr', False, "%%d %s", False)
addFmt ('mk', False, ["%d јануари", "%d февруари", "%d март", "%d април",
...
addFmt ('nl', False, ["%%d %s" % v
                      for v in ["januari", "februari", "maart", "april",
                                "mei", "juni", "juli", "augustus",
                                "september", "oktober", "november",
                                "december"]])
addFmt ('nn', False, ["%%d. %s" % v
                      for v in ["januar", "februar", "mars", "april",
                                "mai", "juni", "juli", "august",
                                "september", "oktober", "november",
                                "desember"]])
addFmt2('no', False, "%%d. %s", False)
addFmt ('oc', False, ["%d de genièr", "%d de febrièr", "%d de març",
                      "%d d'abril", "%d de mai", "%d de junh",
                      "%d de julhet", "%d d'agost", "%d de setembre",
                      "%d d'octobre", "%d de novembre", "%d de decembre"])
addFmt ('os', False, ["%d январы", "%d февралы", "%d мартъийы",
                      "%d апрелы", "%d майы", None, "%d июлы", None,
                      "%d сентябры", None, "%d ноябры", "%d декабры"])
addFmt ('pl', False, ["%d stycznia", "%d lutego", "%d marca",
                      "%d kwietnia", "%d maja", "%d czerwca", "%d lipca",
                      "%d sierpnia", "%d września", "%d października",
                      "%d listopada", "%d grudnia"])
addFmt2('pt', False, "%%d de %s", True)
addFmt2('ro', False, "%%d %s", False)
addFmt ('ru', False, ["%d января", "%d февраля", "%d марта",
                      "%d апреля", "%d мая", "%d июня", "%d июля",
                      "%d августа", "%d сентября", "%d октября",
                      "%d ноября", "%d декабря"])
addFmt2('sco', False, "%%d %s", True)
addFmt2('scn', False, "%%d di %s", False)
addFmt ('se', False, ["ođđajagimánu %d.", "guovvamánu %d.", "njukčamánu %d.",
                      "cuoŋománu %d.", "miessemánu %d.", "geassemánu %d.",
                      "suoidnemánu %d.", "borgemánu %d.", "čakčamánu %d.",
                      "golggotmánu %d.", "skábmamánu %d.", "juovlamánu %d."])
addFmt ('sh', False, makeMonthList("%%d.%d."))
addFmt2('simple', False, "%s %%d", True)
addFmt2('sk', False, "%%d. %s", False)
addFmt2('sl', False, "%%d. %s", False)
addFmt ('sq', False, ["%d Janar", "%d Shkurt", "%d Mars", "%d Prill",
                      "%d Maj", "%d Qershor", "%d Korrik", "%d Gusht",
                      "%d Shtator", "%d Tetor", "%d Nëntor", "%d Dhjetor"])
...
addFmt ('uk', False, ["%d січня", "%d лютого", "%d березня", "%d квітня",
                      "%d травня", "%d червня", "%d липня", "%d серпня",
                      "%d вересня", "%d жовтня", "%d листопада",
                      "%d грудня"])
addFmt ('ur', False, ["%d جنوری", "%d فروری", "%d مارچ", "%d اپریل",
                      "%d مئ", "%d جون", "%d جلائ", "%d اگست", "%d ستمب",
                      "%d اکتوبر", "%d نومب", "%d دسمب"])
addFmt2('vec', False, "%%d de %s", False)
addFmt ('vi', False, makeMonthList("%%d tháng %d"))
addFmt2('vo', False, "%s %%d", False)
addFmt ('zh', False, makeMonthList("%d月%%d日"))

# Walloon names depend on the day number, thus we must generate various
# different patterns
waMonthNames = ["djanvî", "fevrî", "måss", "avri", "may", "djun",
...
for i in (0, 1, 2, 4, 5, 6, 8, 10, 11):
    formats[dayMnthFmts[i]]['wa'] = eval(
        ('lambda m: multi(m, [' +
         '(lambda v: dh_dayOfMnth(v, u"%%dî d\' %s"), lambda p: p == 1),' +
         '(lambda v: dh_dayOfMnth(v, u"%%d d\' %s"),  lambda p: p in [2,3,20,22,23]),' +
         '(lambda v: dh_dayOfMnth(v, u"%%d di %s"),   alwaysTrue)])')
...
for i in (3, 7, 9):
    formats[dayMnthFmts[i]]['wa'] = eval(
        ('lambda m: multi(m, [' +
         '(lambda v: dh_dayOfMnth(v, u"%%dî d\' %s"), lambda p: p == 1),' +
         '(lambda v: dh_dayOfMnth(v, u"%%d d\' %s"),  alwaysTrue)])')
...
for i in range(0, 12):
    formats[dayMnthFmts[i]]['br'] = eval(
        ('lambda m: multi(m, [' +
         '(lambda v: dh_dayOfMnth(v, u"%%dañ %s"), lambda p: p == 1),' +
         '(lambda v: dh_dayOfMnth(v, u"%%d %s"),   alwaysTrue)])')
...
addFmt ('el', True, ["Ιανουάριος %d", "Φεβρουάριος %d", "Μάρτιος %d",
...
addFmt ('fr', True, ["Janvier %d", "Février %d", "Mars %d", "Avril %d",
...
addFmt ('ja', True, ["「最近の出来事」%%d年%d月" % mm for mm in range(1, 13)])
addFmt2('ka', True, "%s, %%d")
addFmt ('ko', True, ["%d년 1월", "%d년 2월", "%d년 3월", "%d년 4월",
                     "%d년 5월", "%d년 6월", "%d년 7월", "%d년 8월",
                     "%d년 9월", "%d년 10월", "%d년 11월", "%d년 12월"])
addFmt ('li', True, ["januari %d", "februari %d", "miert %d", "april %d",
                     "mei %d", "juni %d", "juli %d", "augustus %d",
                     "september %d", "oktober %d", "november %d",
                     "december %d"])
addFmt ('nl', True, ["Januari %d", "Februari %d", "Maart %d", "April %d",
                     "Mei %d", "Juni %d", "Juli %d", "Augustus %d",
                     "September %d", "Oktober %d", "November %d",
                     "December %d"])
addFmt2('pl', True, "%s %%d", True)
addFmt ('scn', True, [None, None, "Marzu %d", None, None, None, None, None,
                      None, None, None, None])
addFmt2('simple', True, "%s %%d", True)
addFmt2('sk', True, "%s %%d")
addFmt2('sv', True, "%s %%d", True)
addFmt2('th', True, "%s พ.ศ. %%T")
addFmt2('tl', True, "%s %%d")
addFmt2('tt', True, "%s, %%d", True)
addFmt ('ur', True, ["%d01مبم", "%d02مبم", "%d03مبم", "%d04مبم",
                     "%d05مبم", "%d06مبم", "%d07مبم", "%d08مبم", "%d09مبم",
                     "%d10مبم", "%d11مبم", "%d12مبم"])
addFmt2('uk', True, "%s %%d", True)
addFmt ('vi', True, makeMonthList("Tháng %d năm %%d"))
addFmt ('zh', True, makeMonthList("%%d年%d月"))
addFmt ('zh-min-nan', True, makeMonthList("%%d nî %d goe̍h"))
...
def getAutoFormat(lang, title, ignoreFirstLetterCase=True):
    ...
    """
    for dictName, dict in list(formats.items()):
        try:
...
def testMapEntry(formatName, showAll=True, value=None):
    ...
    if showAll:
        pywikibot.output(("Processing %s with limits from %d to %d and step %d"
                         % (formatName, start, stop - 1, step)))

    for code, convFunc in list(formats[formatName].items()):
##        import time
            ...
            try:
                ...
                if newValue != value:
                    raise AssertionError(
                        ...
                        % (newValue, value))
                if showAll:
                    pywikibot.output(("date.formats['%s']['%s'](%d): '%s' -> %d"
                                     % (formatName, code, value, convFunc(value),
                                        newValue)))
            except:
                pywikibot.output(("********** Error in date.formats['%s']['%s'](%d)"
...
def test(quick=False, showAll=False):
    ...
    for formatName in formats:
        if quick:
            ...
            pywikibot.output(("'%s' complete." % formatName))
    if quick:
##        print(u'Date module quick consistency test passed')
        pass
    else:
        print('Date module has been fully tested')
